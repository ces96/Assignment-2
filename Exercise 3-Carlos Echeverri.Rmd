...
*****
```{r}
double_odd <- function(x){
  ifelse(abs(x)%%2 == 0, x, 2*x)
}
```
...
```{r}
tmpFn4 <- function(n, k){
  M <- matrix(rep(0, times = n^2), nrow = n)
  for (i in 1:n){
    if(i==1){
      M[i,i]= k; M[i+1,i]= 1;
    } else if (i==n){
...
weekday <- function(day, month, year){
  ...
  c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday")[1+f%%7]
}
```
*****
...
No, it only works one date at a time, for example:
